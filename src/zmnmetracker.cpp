...
CzmnmeTracker::CzmnmeTracker(std::string strWalletFile)
{
...
CzmnmeTracker::~CzmnmeTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzmnmeTracker::Init()
{
...
bool CzmnmeTracker::Archive(CMintMeta& meta)
{
...
bool CzmnmeTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzmnmeTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzmnmeTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzmnmeTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzmnmeTracker::GetSerialHashes()
{
...
CAmount CzmnmeTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzmnmeTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzmnmeTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzmnmeTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzmnmeTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzmnmeTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzmnmeTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzmnmeTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzmnmeTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzmnmeTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzmnmeTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzmnmeTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzmnmeTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzmnmeTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzmnmeTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzmnmeTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzmnmeTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzmnme from DB\n", __func__, listDeterministicDB.size());
    }
