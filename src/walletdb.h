...
public:
    ...
    bool WriteZMNMESeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZMNMESeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZMNMESeed_deprecated(uint256& seed);
    bool EraseZMNMESeed();
    bool EraseZMNMESeed_deprecated();

    bool WriteZMNMECount(const uint32_t& nCount);
    bool ReadZMNMECount(uint32_t& nCount);
