...
#ifndef MasterNodesME_ZMNMEWALLET_H
#define MasterNodesME_ZMNMEWALLET_H
...
public:
    CzmnmeWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZMNME(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZMNME(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //MasterNodesME_ZMNMEWALLET_H
