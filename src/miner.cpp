...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

	{
		...
		for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
			...
			for (const CTxIn& txin : tx.vin) {
				...
				if (tx.IsZerocoinSpend()) {
					...
					//Priority = (age^6+100000)*amount - gives higher priority to zmnmes that have been in mempool long
					//and higher priority to zmnmes that are large in value
					...
					// zmnme spends can have very large priority, use non-overflowing safe functions
		...
		while (!vecPriority.empty()) {
			...
			// double check that there are no double spent zmnme spends in this block or tx
			if (tx.IsZerocoinSpend()) {
				int nHeightTx = 0;
				...
				//This zmnme serial has already been included in the block, do not add this tx.
				if (fDoubleSerial)
					continue;
			}
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
	...
	// Found a solution
	{
		...
		if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
			return error("MasterNodesMEMiner : generated block is stale");
	}
	...
	if (!ProcessNewBlock(state, NULL, pblock)) {
		if (pblock->IsZerocoinStake())
			pwalletMain->zmnmeTracker->RemovePending(pblock->vtx[1].GetHash());
		return error("MasterNodesMEMiner : ProcessNewBlock, block not accepted");
	}
...
{
	LogPrintf("MasterNodesMEMiner started\n");
	SetThreadPriority(THREAD_PRIORITY_LOWEST);
	RenameThread("masternodesme-miner");
	...
	while (fGenerateBitcoins || fProofOfStake) {
		...
		if (fProofOfStake) {
			...
			if (pblock->IsZerocoinStake()) {
				...
				if (!pwallet->GetZerocoinKey(bnSerial, key)) {
					LogPrintf("%s: failed to find zmnme with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
					continue;
				}

				//Sign block with the zmnme key
				if (!SignBlockWithKey(*pblock, key)) {
					LogPrintf("BitcoinMiner(): Signing new block with zmnme key failed \n");
					continue;
				}
			}
		...
		LogPrintf("Running MasterNodesMEMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
