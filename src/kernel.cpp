...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZMNMEStake(spend));
    } else {
        ...
        CMNMEStake* MNMEInput = new CMNMEStake();
        MNMEInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(MNMEInput);
    }
