...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total masternodesme balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in masternodesme/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in masternodesme/kb\n"
...
UniValue mnmeync(const UniValue& params, bool fHelp)
{
    std::string strMode;
    if (params.size() == 1)
    ...
    if (fHelp || params.size() != 1 || (strMode != "status" && strMode != "reset")) {
        throw runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("mnmeync", "\"status\"") + HelpExampleRpc("mnmeync", "\"status\""));
    }
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"masternodesmeaddress\"\n"
            "\nReturn information about the given masternodesme address.\n"

            "\nArguments:\n"
            "1. \"masternodesmeaddress\"     (string, required) The masternodesme address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"masternodesmeaddress\", (string) The masternodesme address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: MasterNodesME address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are masternodesme addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) masternodesme address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"masternodesmeaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"masternodesmeaddress\"  (string, required) The masternodesme address to use for the signature.\n"
...
UniValue getstakingstatus(const UniValue& params, bool fHelp)
{
    ...
    obj.push_back(Pair("mnmeync", masternodeSync.IsSynced()));
