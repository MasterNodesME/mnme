...
#include "config/masternodesme-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("masternodesme-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "masternodesme.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "masternodesmed.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the MNME and zmnme money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in MNME/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in MNME/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, masternodesme, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in MNME/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all MasterNodesME specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-MNMEstake=<n>", strprintf(_("Enable or disable staking functionality for MNME inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 44111, 32823));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
		   ...
		   "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The MasterNodesME Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("masternodesme-loadblk");
...
/** Sanity checks
 *  Ensure that MasterNodesME is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. MasterNodesME is shutting down."));
...
#endif
    // Make sure only a single MasterNodesME process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MasterNodesME is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("MasterNodesME version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //MasterNodesME: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //MasterNodesME specific: zerocoin and spork DB's
                ...
                // MasterNodesME: load previous sessions sporks if we have them.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MasterNodesME") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart MasterNodesME to complete") << "\n";
        ...
        zwalletMain = new CzmnmeWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zmnmeWallet


        //Load zerocoin mint hashes to memory
        pwalletMain->zmnmeTracker->Init();
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeMasterNodesMEAmount = 2;
...
//    }
//
//    nAnonymizeMasterNodesMEAmount = GetArg("-anonymizemasternodesmeamount", 0);
//    if (nAnonymizeMasterNodesMEAmount > 999999) nAnonymizeMasterNodesMEAmount = 999999;
//    if (nAnonymizeMasterNodesMEAmount < 2) nAnonymizeMasterNodesMEAmount = 2;
    ...
    LogPrintf("Anonymize MasterNodesME Amount %d\n", nAnonymizeMasterNodesMEAmount);
    ...
    /* Denominations
       ...
       For example:
       1MNME +1000 == (.1MNME +100)*10
       10MNME +10000 == (1MNME +1000)*10
    */
