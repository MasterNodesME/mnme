...
CzmnmeWallet::CzmnmeWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zmnme seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMNMESeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMNMESeed_deprecated()) {
                    LogPrintf("%s: Updated zmnme seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zmnme seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zmnme wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzmnmeWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMNMECount(nCountLastUsed);
    else if (!walletdb.ReadZMNMECount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzmnmeWallet::Lock()
{
    seedMaster = 0;
}

void CzmnmeWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzmnmeWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMNME(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzmnmeWallet::LoadMintPoolFromDB()
{
...
void CzmnmeWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzmnmeWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzmnmeWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zmnmeTracker->HasPubcoinHash(pMint.first)) {
...
bool CzmnmeWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMNME(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zmnmeTracker which also adds to database
    pwalletMain->zmnmeTracker->Add(dMint, true);
...
void CzmnmeWallet::SeedToZMNME(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzmnmeWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzmnmeWallet::GenerateDeterministicZMNME(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzmnmeWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzmnmeWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
