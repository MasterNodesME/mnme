...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::MNME);
    ...
    if (!settings.contains("nAnonymizeMasterNodesMEAmount"))
        settings.setValue("nAnonymizeMasterNodesMEAmount", 1000);

    nAnonymizeMasterNodesMEAmount = settings.value("nAnonymizeMasterNodesMEAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeMasterNodesMEAmount"))
        SoftSetArg("-anonymizemasternodesmeamount", settings.value("nAnonymizeMasterNodesMEAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in masternodesme.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMasterNodesMEAmount:
            return QVariant(nAnonymizeMasterNodesMEAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMasterNodesMEAmount:
            nAnonymizeMasterNodesMEAmount = value.toInt();
            settings.setValue("nAnonymizeMasterNodesMEAmount", nAnonymizeMasterNodesMEAmount);
            emit anonymizeMasterNodesMEAmountChanged(nAnonymizeMasterNodesMEAmount);
            break;
        case CoinControlFeatures:
