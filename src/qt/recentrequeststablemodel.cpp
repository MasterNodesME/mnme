...
RecentRequestsTableModel::RecentRequestsTableModel(CWallet* wallet, WalletModel* parent) : walletModel(parent)
{
    ...
    /* These columnme must match the indices in the ColumnIndex enumeration */
    columnme << tr("Date") << tr("Label") << tr("Address") << tr("Message") << getAmountTitle();
...
QVariant RecentRequestsTableModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    if (orientation == Qt::Horizontal) {
        if (role == Qt::DisplayRole && section < columnme.size()) {
...
{
    columnme[Amount] = getAmountTitle();
...
void RecentRequestsTableModel::sort(int column, Qt::SortOrder order)
{
    ...
    emit dataChanged(index(0, 0, QModelIndex()), index(list.size() - 1, NUMBER_OF_COLUMNME - 1, QModelIndex()));
}
