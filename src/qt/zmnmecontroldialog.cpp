...
#include "ui_zmnmecontroldialog.h"
...
std::set<std::string> ZMNMEControlDialog::setSelectedMints;
std::set<CMintMeta> ZMNMEControlDialog::setMints;

ZMNMEControlDialog::ZMNMEControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZMNMEControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZMNMEControlDialog::~ZMNMEControlDialog()
{
    delete ui;
}

void ZMNMEControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMNMEControlDialog::updateList()
{
...
void ZMNMEControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMNMEControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZMNME_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMNMEControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZMNMEControlDialog::GetSelectedMints()
{
...
void ZMNMEControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
