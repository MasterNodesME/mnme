...
#define URI_SCHEME "masternodesme"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a MasterNodesME address (e.g. %1)").arg("C6M1kT6PQaieDqNv2ajcTJtcfip2krta6a"));
...
{
    // return if URI is not valid or is no MasterNodesME: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::MNME, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert masternodesme:// to masternodesme:
    //
    //    Cannot handle this later, because masternodesme:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::MNME, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open masternodesme.conf with the associated application */
...
int TableViewLastColumnResizingFixer::getColumnmeWidth()
{
    int nColumnmeWidthSum = 0;
    for (int i = 0; i < columnCount; i++) {
        nColumnmeWidthSum += tableView->horizontalHeader()->sectionSize(i);
    }
    return nColumnmeWidthSum;
}
...
int TableViewLastColumnResizingFixer::getAvailableWidthForColumn(int column)
{
    ...
    if (nTableWidth > 0) {
        int nOtherColsWidth = getColumnmeWidth() - tableView->horizontalHeader()->sectionSize(column);
...
// Make sure we don't make the columnme wider than the tables viewport width.
void TableViewLastColumnResizingFixer::adjustTableColumnmeWidth()
{
    ...
    int nTableWidth = tableView->horizontalHeader()->width();
    int nColsWidth = getColumnmeWidth();
...
{
    if ((getColumnmeWidth() - this->tableView->horizontalHeader()->width()) != 0) {
        disconnectViewHeadersSignals();
        resizeColumn(secondToLastColumnIndex, getAvailableWidthForColumn(secondToLastColumnIndex));
        connectViewHeadersSignals();
    }
 ...
 * the resize modes of the last 2 columnme of the table and
 */
TableViewLastColumnResizingFixer::TableViewLastColumnResizingFixer(QTableView* table, int lastColMinimumWidth, int allColsMinimumWidth) : tableView(table),
                                                                                                                                          ...
                                                                                                                                          allColumnmeMinimumWidth(allColsMinimumWidth)
{
    columnCount = tableView->horizontalHeader()->count();
    lastColumnIndex = columnCount - 1;
    secondToLastColumnIndex = columnCount - 2;
    tableView->horizontalHeader()->setMinimumSectionSize(allColumnmeMinimumWidth);
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "MasterNodesME.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for MasterNodesME.lnk
...
{
    return GetAutostartDir() / "masternodesme.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a masternodesme.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=MasterNodesME\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the masternodesme app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add masternodesme app to startup item list
