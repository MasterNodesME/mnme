...
void FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, bool fProofOfStake, bool fZMNMEStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    ...
    if (IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS) && budget.IsBudgetPaymentBlock(pindexPrev->nHeight + 1)) {
        ...
        masternodePayments.FillBlockPayee(txNew, nFees, fProofOfStake, fZMNMEStake);
    }
}
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake, bool fZMNMEStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    if (!pindexPrev) return;
    ...
    for (unsigned int masternodePhase = 1; masternodePhase <= Params ().getMasternodePhaseCount (pindexPrev->nHeight + 1); masternodePhase++) {
        ...
        CAmount masternodePayment = GetMasternodePayment (pindexPrev->nHeight + 1, masternodePhase, blockValue, 0, fZMNMEStake);
