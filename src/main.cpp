...
#if defined(NDEBUG)
#error "MasterNodesME cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uMNME) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
	...
	if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
		return error("%s : zmnme spend with serial %s is already in block %d\n", __func__,
...
bool ContextualCheckZerocoinSpendNoSerialCheck(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
	...
	//Check to see if the zmnme is properly signed
	if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
		if (!spend.HasValidSignature())
			return error("%s: V2 zmnme spend does not have a valid signature", __func__);

		libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
		if (tx.IsCoinStake())
			expectedType = libzerocoin::SpendType::STAKE;
		if (spend.getSpendType() != expectedType) {
			return error("%s: trying to spend zmnme without the correct spend type. txid=%s", __func__,
				tx.GetHash().GetHex());
		}
	}
	...
	if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
		...
		return error("%s : zmnme spend with serial %s from tx %s is not in valid range\n", __func__,
...

	{
		...
		if (tx.IsZerocoinSpend()) {
			...
			if (IsTransactionInChain(tx.GetHash(), nHeightTx))
				return state.Invalid(error("AcceptToMemoryPool : zmnme spend tx %s already in block %d",
			...
			for (const CTxIn& txIn : tx.vin) {
				...
				if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
					return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
						tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zmnme");
			}
		}
		else {
			LOCK(pool.cs);
			...
			// Check that zmnme mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 45000 && nHeight > 30000) {
		nSubsidy = 3 * COIN;
	}
	else if (nHeight <= 60000 && nHeight > 45000) {
		nSubsidy = 4 * COIN;
	}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZMNMEStake)
{
    int64_t ret = 0;
...
int64_t GetMasternodePayment (int nHeight, unsigned int mnLevel, int64_t blockValue, int nMasternodeCount, bool isZMNMEStake) {
  return GetMasternodePayment (nHeight, blockValue, nMasternodeCount, isZMNMEStake) * Params ().getMasternodePhaseWeight (mnLevel, nHeight) / Params ().getMasternodePhaseWeight (0, nHeight);
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
	...
	for (int i = block.vtx.size() - 1; i >= 0; i--) {
		...
		* note we only undo zerocoin databasing in the following statement, value to and from MasterNodesME
...
{
	RenameThread("masternodesme-scriptch");
	scriptcheckqueue.Thread();
}

void RecalculateZMNMEMinted()
{
...
void RecalculateZMNMESpent()
{
	...
	while (true) {
		...
		//Remove spends from zmnme supply
...
bool RecalculateMNMESupply(int nHeightStart)
{
...
{
	// MasterNodesME: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZMNMESupply(const CBlock& block, CBlockIndex* pindex)
{
	std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
	...
	if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
		RecalculateZMNMEMinted();
		RecalculateZMNMESpent();
		RecalculateMNMESupply(Params().Zerocoin_StartHeight());
	}

	//Track zmnme money supply in the block index
	if (!UpdateZMNMESupply(block, pindex))
		return state.DoS(100, error("%s: Failed to calculate new zmnme supply for block=%s height=%d", __func__,
	...
	//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zMNMESpent: %s\n",
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
	...
	for (const CTransaction& tx : block.vtx) {
		...
		// double check that there are no double spent zmnme spends in this block
		if (tx.IsZerocoinSpend()) {
			for (const CTxIn& txIn : tx.vin) {
				if (txIn.scriptSig.IsZerocoinSpend()) {
					libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
					if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
						return state.DoS(100, error("%s : Double spending of zmnme serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
	if (nHeight < Params().Zerocoin_Block_V2_Start())
		return error("%s: zmnme stake block is less than allowed start height", __func__);

	if (CZMNMEStake* zmnme = dynamic_cast<CZMNMEStake*>(stake)) {
		CBlockIndex* pindexFrom = zmnme->GetIndexFrom();
		if (!pindexFrom)
			return error("%s: failed to get index associated with zmnme stake checksum", __func__);

		if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
			return error("%s: zmnme stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
		...
		uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zmnme->GetValue()));
		if (nChecksum200 != zmnme->GetChecksum())
			return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zmnme->GetChecksum(), nChecksum200);
	}
	else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	if (block.IsProofOfStake()) {
		...
		if (stake->IsZMNME() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
			return state.DoS(100, error("%s: staked zmnme fails context checks", __func__));
	...
	if (isPoS) {
		...
		// Inputs
		std::vector<CTxIn> MNMEInputs;
		std::vector<CTxIn> zmnmeInputs;

		for (CTxIn stakeIn : stakeTxIn.vin) {
			if (stakeIn.scriptSig.IsZerocoinSpend()) {
				zmnmeInputs.push_back(stakeIn);
			}
			else {
				MNMEInputs.push_back(stakeIn);
			}
		}
		const bool hasMNMEInputs = !MNMEInputs.empty();
		const bool hasZMNMEInputs = !zmnmeInputs.empty();
		...
		for (CTransaction tx : block.vtx) {
			for (CTxIn in : tx.vin) {
				...
				if (hasMNMEInputs)
					...
					for (CTxIn MNMEIn : MNMEInputs) {
						if (MNMEIn.prevout == in.prevout) {
		...
		if (isBlockFromFork) {
			...
			// Now that this loop if completed. Check if we have zmnme inputs.
			if (hasZMNMEInputs) {
				for (CTxIn zMNMEInput : zmnmeInputs) {
					CoinSpend spend = TxInToZerocoinSpend(zMNMEInput);
					...
					if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
						return state.DoS(100, error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
							stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zmnme");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
	...
	if (nMints || nSpends)
		LogPrintf("%s : block contains %d zmnme mints and %d zmnme spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	if (strCommand == "version") {
		...
		// MasterNodesME: We use certain sporks during IBD, so check to see if they are
