...
enum AvailableCoinsType {
    ...
    EXCEPT_DENOMINATED_OR_MASTERNODE_COLLATERAL = 4, // ONLY_NONDENOMINATED and not 15000 MNME at the same time
    ONLY_MASTERNODE_COLLATERAL = 5,       // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zmnme send
enum ZerocoinSpendStatus {
    ZMNME_SPEND_OKAY = 0,                            // No error
    ZMNME_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZMNME_WALLET_LOCKED = 2,                         // Wallet was locked
    ZMNME_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZMNME_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZMNME_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZMNME_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZMNME_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZMNME_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZMNME_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZMNME_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZMNME_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZMNME_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZMNME_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZMNME_SPENT_USED_ZMNME = 14,                      // Coin has already been spend
    ZMNME_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZMNME_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZMNMEBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZMNMEOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzmnmeAuto) const;
    ...
    */
    ...
    CzmnmeWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzmnmeTracker> zmnmeTracker;
    ...
    void setZWallet(CzmnmeWallet* zwallet)
    {
        zwalletMain = zwallet;
        zmnmeTracker = std::unique_ptr<CzmnmeTracker>(new CzmnmeTracker(strWalletFile));
    }

    CzmnmeWallet* getZWallet() { return zwalletMain; }
    ...
    /** zmnme reset */
    boost::signals2::signal<void()> NotifyzmnmeReset;
