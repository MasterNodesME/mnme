...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zmnmeTracker = unique_ptr<CzmnmeTracker>(new CzmnmeTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZMNME_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZMNME_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
